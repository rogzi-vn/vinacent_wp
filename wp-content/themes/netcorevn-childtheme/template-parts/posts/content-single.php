...
<?php if ( get_theme_mod( 'blog_single_footer_meta', 1 ) ) : ?>
	<footer class="entry-meta text-<?php echo get_theme_mod( 'blog_posts_title_align', 'center' ); ?>">
		...
		if ( '' != $tag_list ) {
			$meta_text = __( 'Bài viết này đã được đăng trong chuyên mục %1$s và được gắn thẻ là %2$s.', 'flatsome' );
		} else {
			$meta_text = __( 'Bài viết này đã được đăng trong chuyên mục %1$s. Đánh dấu <a href="%3$s" title="Permalink to %4$s" rel="bookmark">liên kết cố định</a>.', 'flatsome' );
		}
